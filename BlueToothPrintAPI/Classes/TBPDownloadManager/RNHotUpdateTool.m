...
#import "PRHotUpdateTool.h"
#import <SSZipArchive/SSZipArchive.h>
#import <CommonCrypto/CommonDigest.h>

@interface PRHotUpdateTool ()
/// 下载和请求json工具类
@property (nonatomic,strong) PRDowloadTool *downloadTool;

@end
@implementation PRHotUpdateTool
static PRHotUpdateTool *downLoadTool;//单例
...
#pragma mark - 获取MD5
- (NSString *)getMd5WithFlie:(NSString *)fliePath
{
...
#pragma mark - 懒加载
- (PRDowloadTool *)downloadTool
{
    if (_downloadTool == nil) {
        _downloadTool = [[PRDowloadTool alloc]init];
    }
...
+ (instancetype)sharedRNHotUpdateTool{
    ...
    dispatch_once(&token, ^{
        if (!downLoadTool) {
            downLoadTool = [[PRHotUpdateTool alloc]init];
